...
export default function SocialSidebar({ locale = 'en' }: { locale?: 'en' | 'fr' }) {
  const isFrench = locale === 'fr';
  const frenchWhatsAppUrl = `${site.whatsappUrl}?text=${encodeURIComponent(
  ...
  return (
    <div className="fixed right-0 top-1/2 z-50 hidden -translate-y-1/2 transform flex-col gap-0 xl:flex">
      {/* WhatsApp */}
      <a
        href={isFrench ? frenchWhatsAppUrl : site.whatsappUrl}
        target="_blank"
      ...
      <div className="relative group">
        <button
          ...
          title="WeChat"
          aria-label={isFrench ? 'Code QR WeChat' : 'WeChat QR Code'}
        >
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="pointer-events-none absolute right-full top-1/2 mr-3 w-56 origin-right -translate-y-1/2 translate-x-3 scale-95 overflow-hidden rounded-lg bg-white opacity-0 shadow-xl ring-1 ring-black/5 transition-all duration-300 ease-out group-hover:pointer-events-auto group-hover:translate-x-0 group-hover:scale-100 group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:translate-x-0 group-focus-within:scale-100 group-focus-within:opacity-100">
          <div className="p-4 text-center">
            <div className="mb-3 text-sm font-semibold text-gray-800">{isFrench ? 'Scanner le code QR WeChat' : 'Scan WeChat QR Code'}</div>
            <Image
              src="/wechat-qrcode.jpg"
              alt={isFrench ? 'Code QR WeChat' : 'WeChat QR Code'}
              width={176}
              height={176}
              className="mx-auto h-44 w-44 object-contain"
            />
            <div className="mt-2 text-xs text-gray-500">{isFrench ? 'Ajoutez-moi sur WeChat' : 'Add me on WeChat'}</div>
          </div>
        </div>
      </div>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        aria-label={isFrench ? 'Voir le profil LinkedIn de Scott Qi' : 'View Scott Qi on LinkedIn'}
